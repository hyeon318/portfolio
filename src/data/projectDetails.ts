...
export const projectData: Record<string, ProjectDetailData> = {
  "ocr-note": {
    ...
    reflection:
      ...
      "개발 과정에서는 백엔드를 먼저 구축한 뒤, 프론트엔드에서 <strong>generate-api</strong> 라이브러리를 활용해 API 타입 정의와 axios 요청 코드를 자동 생성하였습니다. 이를 통해 <strong>프론트/백 간 불일치를 방지하고, 반복적인 코드 작성 부담을 줄여</strong> 개발 속도를 높일 수 있었습니다.<br/>" +
      "또한 <strong>OpenAI</strong>와 연동하여 다양한 프롬프트 실험을 진행했으며, 응답의 일관성을 확보하는 과정이 주요 과제 중 하나였습니다. 이를 해결하기 위해 <strong>JSON 형식 강제와 규칙 기반 프롬프트 설계(페르소나·넘버링 규칙 적용)</strong>를 통해 출력 품질과 활용 만족도를 높였습니다.<br/>" +
